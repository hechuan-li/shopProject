<template>
  <div style="padding: 12px">
    ...
    <!-- 商品购买模块 -->
    <div class="box">
      <p style="padding: 5px 0; border-bottom: 1px solid #ccc">
        {{ goodsDetail.title }}
      </p>
      <div style="padding: 0px 10px; font-size: 14px; color: #999">
        ...
        <div style="display: flex; align-items: center; margin-bottom: 10px">
          <span>购买数量</span
          ><van-stepper
            v-model="targetGoods.buy_num"
            min="1"
            :max="goodsDetail.stock_quantity"
          />
        </div>
        <van-button type="info" size="normal">立即购买</van-button>
        <van-button type="danger" size="normal" @click="handleAddGoodsToShopCar"
          >加入购物车</van-button>
      </div>
    </div>
    <!-- 商品参数模块 -->
    <div class="box">
      ...
      <div
        style="padding: 0px 10px; font-size: 14px; color: #999; border-bottom: 1px solid #ccc; margin-bottom: 10px"
      >
        <p>商品货号: {{ goodsDetail.goods_no }}</p>
        <p>库存情况: {{ goodsDetail.stock_quantity }}件</p>
        <p>上架时间: {{ goodsDetail.add_time }}</p>
      </div>
      <van-button plain type="info" block @click="handleToGoodsInfo"
        >图文介绍</van-button
      >
      <van-button plain type="danger" block @click="handleToGoodsComments"
        >商品评论</van-button
      >
    </div>
  </div>
  ...
  methods: {
    ...
    async getGoodsDetail() {
      const { data: res } = await this.$http.get(
        '/api/goods/getinfo/' + this.targetGoods.goodsId
      )
    ...
    async getGoodsImgs() {
      const { data: res } = await this.$http.get(
        '/api/getthumimages/' + this.targetGoods.goodsId
      )
    ...
    handleToGoodsInfo() {
      this.$router.push({
        path: '/goodsInfo',
        query: { id: this.targetGoods.goodsId }
      })
    },
    // 跳转到商品评论页
    ...
    async getShopCarInfo() {
      ...
      window.sessionStorage.setItem('shopItem', this.goodsItem)
    }
  }
}
</script>
